...
def delete_transfer_item(item_id):
    ...
    try:
        ...
        logging.info(f" Item {item_code} deleted from inventory transfer {transfer_id}")
...
def serial_index():
    ...
    # Build base query with eager loading of items relationship
    from sqlalchemy.orm import joinedload
    query = SerialNumberTransfer.query.options(joinedload(SerialNumberTransfer.items))
...
def serial_detail(transfer_id):
    ...
    # Load transfer with eager loading of items and their serial numbers
    from sqlalchemy.orm import joinedload
    transfer = SerialNumberTransfer.query.options(
        joinedload(SerialNumberTransfer.items).joinedload(SerialNumberTransferItem.serial_numbers)
...
def serial_add_item(transfer_id):
    ...
    try:
        ...
        if existing_item:
            logging.warning(f" Duplicate item prevention: {item_code} already exists in transfer {transfer_id}")
            return jsonify({
                'success': False, 
        ...
        for batch_index in range(total_batches):
            ...
            for serial_number in batch:
                try:
                    ...
                    if is_duplicate:
                        ...
                        failed_count += 1
                        logging.warning(f" Duplicate serial number {serial_number} marked as invalid")
                    else:
                    ...
                    if len(serial_numbers) >= 1000:
                        ...
                        if current_item % 50 == 0:  # Every 50 items
                            ...
                            logging.debug(f" SAP timeout prevention: Processed {current_item}/{len(serial_numbers)} items")
                    ...
                    #     logging.warning(f" Duplicate serial number {serial_number} detected via database error, skipping")
                    #     continue
            ...
            try:
                ...
                logging.info(f" BATCH {batch_index + 1}/{total_batches} ({progress_percent:.1f}%) | VALIDATED {validated_count}/{len(serial_numbers)} | Batch: {batch_success_rate:.1f}% | Overall: {overall_success_rate:.1f}%")
                        ...
                        try:
                            ...
                            for serial_number in sub_batch:
                                try:
                                    ...
                                    logging.warning(f" Individual item error for {serial_number}: {str(individual_error)}")
...
def serial_delete_item(item_id):
    ...
    try:
        ...
        logging.info(f" Item {item_code} deleted from serial number transfer {transfer_id}")
        return jsonify({'success': True, 'message': f'Item {item_code} deleted'})
...
def serial_delete_serial_number(serial_id):
    ...
    try:
        ...
        logging.info(f" Serial number {serial_number} deleted from item {item_code} in transfer {transfer_id}")
        return jsonify({
            'success': True, 
...
def serial_add_more_serials(item_id):
    ...
    try:
        ...
        if total_valid == expected_quantity:
            ...
            message = f' Added {len(new_serials)} serial numbers. Total: {total_valid}/{expected_quantity} valid serials. Need {missing} more.'
        else:
            extra = total_valid - expected_quantity
            message = f' Added {len(new_serials)} serial numbers. Total: {total_valid}/{expected_quantity} valid serials. {extra} extra valid serials found.'
...
@transfer_bp.route('/validate_batch_serials', methods=['POST'])
@login_required
def validate_batch_serials():
    """API endpoint to validate multiple serial numbers in batch"""
    try:
        # Get form data
        serial_numbers_data = request.form.get('serial_numbers', '')
        item_code = request.form.get('item_code', '').strip()
        warehouse_code = request.form.get('warehouse_code', '').strip()
        
        # Parse serial numbers from JSON string or newline-separated text
        try:
            import json
            serial_numbers = json.loads(serial_numbers_data)
            if isinstance(serial_numbers, str):
                serial_numbers = [sn.strip() for sn in serial_numbers.split('\n') if sn.strip()]
        except (json.JSONDecodeError, TypeError):
            # Fallback to text parsing
            serial_numbers = [sn.strip() for sn in serial_numbers_data.split('\n') if sn.strip()]
        ...
        if len(serial_numbers) > 2000:
            return jsonify({
                'success': False,
                'error': f'Too many serial numbers ({len(serial_numbers)}). Maximum batch size is 2000.'
            }), 400
        
        # Use the existing batch validation function
        results = validate_batch_series_with_warehouse_sap(serial_numbers, item_code, warehouse_code)
        
        return jsonify(results)
        
    except Exception as e:
        logging.error(f"Error in batch serial validation API: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'Batch validation error: {str(e)}'
        }), 500

@transfer_bp.route('/serial/pre-validate', methods=['POST'])
@login_required
def pre_validate_serials_api():
    """Pre-validate serial numbers against SAP without saving to database
    
    This endpoint validates serial numbers and checks quantity matching
    before any database operations to improve performance for large batches.
    """
    try:
        # Get form data
        item_code = request.form.get('item_code', '').strip()
        expected_quantity = request.form.get('expected_quantity', '').strip()
        ...
        if not all([item_code, expected_quantity, serial_numbers_text]):
            return jsonify({
                'success': False,
                'error': 'Item code, expected quantity, and serial numbers are required'
            }), 400
        ...
        try:
            ...
            if expected_qty <= 0:
                return jsonify({
                    'success': False,
                    'error': 'Expected quantity must be a positive number'
                }), 400
        except (ValueError, TypeError):
            return jsonify({
                'success': False,
                'error': 'Invalid expected quantity format'
            }), 400
        ...
        logging.error(f"Error in pre-validation API: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'Pre-validation error: {str(e)}'
        }), 500

@transfer_bp.route('/serial/<int:transfer_id>/add_validated_item', methods=['POST'])
@login_required
def serial_add_validated_item(transfer_id):
    """Add item with pre-validated serial numbers to Serial Number Transfer
    
    This endpoint only saves already validated serial numbers to the database,
    improving performance by skipping SAP validation during database operations.
    """
    try:
        transfer = SerialNumberTransfer.query.get_or_404(transfer_id)
        ...
        if not all([item_code, item_name, expected_quantity, validated_serials_json]):
        ...
        try:
            expected_qty = int(expected_quantity)
            if expected_qty <= 0:
                return jsonify({'success': False, 'error': 'Expected quantity must be positive'}), 400
        except (ValueError, TypeError):
            return jsonify({'success': False, 'error': 'Invalid expected quantity format'}), 400
        
        # Parse validated serials JSON
        import json
        try:
        ...
        # Verify we have the expected number of valid serials
        valid_serials = [s for s, data in validated_serials.items() if data.get('valid')]
        if len(valid_serials) != expected_qty:
            return jsonify({
                'success': False, 
                'error': f'Quantity mismatch: Expected {expected_qty} but got {len(valid_serials)} valid serials'
            }), 400
        ...
        existing_item = SerialNumberTransferItem.query.filter(
            SerialNumberTransferItem.serial_transfer_id == transfer_id,
            func.upper(func.trim(SerialNumberTransferItem.item_code)) == item_code_clean
        ).first()
        
        if existing_item:
            return jsonify({
                'success': False,
                'error': f'Item {item_code} already exists in this transfer. Please delete the existing item first or use a different item code.'
            }), 400
        ...
        # Add validated serial numbers to database
        added_serials = []
        for serial_number in valid_serials:
            validation_data = validated_serials[serial_number]
            
            serial_record = SerialNumberTransferSerial(
                transfer_item_id=transfer_item.id,
                serial_number=serial_number,
                internal_serial_number=validation_data.get('SerialNumber', serial_number),
                system_serial_number=validation_data.get('SystemNumber'),
                is_validated=True,  # Already validated
        ...
        logging.info(f"Added item {item_code} with {len(added_serials)} pre-validated serial numbers to transfer {transfer.transfer_number}")
        
        return jsonify({
            'success': True,
            'message': f'Item {item_code} added successfully with {len(added_serials)} validated serial numbers',
            'item_id': transfer_item.id,
            'serial_count': len(added_serials)
        })
        
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error adding validated item to transfer {transfer_id}: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
